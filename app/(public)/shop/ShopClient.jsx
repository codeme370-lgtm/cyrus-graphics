...
import { ArrowRight, Heart, LayoutGrid, List, ShoppingCart, Star, ChevronDown, X, Filter } from 'lucide-react'
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="mt-8 grid gap-6">
          {/* Left filters hidden (use drawer) */}
          ...
          {/* Drawer toggles (visible on all sizes) */}
          <div className="flex gap-2 mb-4">
            <button onClick={() => setFiltersOpen(true)} className="inline-flex items-center gap-2 rounded-3xl bg-violet-500 px-4 py-2 text-sm font-semibold text-white">
              <Filter size={16} /> Filters
            </button>
            <button onClick={() => setInfoOpen(true)} className="inline-flex items-center gap-2 rounded-3xl bg-slate-900 px-4 py-2 text-sm text-slate-300">
              More
            </button>
          </div>
          ...
          <SideDrawer open={infoOpen} onClose={() => setInfoOpen(false)}>
          
            {/* reuse right-column content in mobile drawer */}
            <RightPanel topCategories={topCategories} />
          </SideDrawer>
...
function FilterPanel({ selectedCategory, setSelectedCategory, selectedBrand, setSelectedBrand, selectedRating, setSelectedRating, priceRange, setPriceRange, categories, brands, ratings, availabilities }) {
  return (
    <div className="rounded-[32px] border border-slate-800 bg-slate-950/80 p-6 shadow-[0_20px_60px_-20px_rgba(15,23,42,0.8)]">
      <div className="flex items-center justify-between">
...
function RightPanel({ topCategories }) {
  return (
